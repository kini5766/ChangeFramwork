...
void SetAnimationWorld2(inout matrix world, VertexModel input)
{
	matrix transform = 0;
	...
	else
	{
		...
		[unroll(4)]
		for (int i = 0; i < 4; i++)  // int는 전역변수
			transform += mul(weights[i], SkinningBoneTransforms[indices[i]]);
...
MeshOutput VS_Animation2(VertexModel input)
{
	MeshOutput output;

	SetAnimationWorld2(World, input);
