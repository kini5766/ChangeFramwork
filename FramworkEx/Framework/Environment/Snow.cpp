...
Snow::Snow(const Vector3 & extent, UINT count, wstring file)
	: Renderer(L"21_Snow.fx"), drawCount(count)
{
	desc.Extent = extent;
...
void Snow::Update()
{
	...
	ImGui::SliderFloat3("Velocity", desc.Velocity, -0.200f, 0.200f);
	ImGui::ColorEdit3("Color", desc.Color);
	ImGui::SliderFloat("Distance", &desc.DrawDistance, 0, desc.Extent.z * 2.0f);
	ImGui::InputFloat("Turblulence", &desc.Turblulence, 0.1f);
