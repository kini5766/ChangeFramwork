...
void MeshSphere::Create()
{
	...
	for (UINT i = 1; i <= stackCount - 1; i++)
	{
		...
		for (UINT j = 0; j <= sliceCount; j++)
		{
			...
			D3DXVECTOR2 uv = D3DXVECTOR2(theta / (Math::PI * 2), phi / Math::PI); // Uv
			v.push_back(MeshVertex(p.x, p.y, p.z, uv.x, uv.y, n.x, n.y, n.z)); //Uv
		}
	}
	v.push_back(MeshVertex(0, -radius, 0, 0, 0, 0, -1, 0)); //Uv
