...
void MeshCylinder::Create()
{
	...
	for (UINT i = 0; i < ringCount; i++)
	{
		...
		for (UINT k = 0; k <= sliceCount; k++)
		{
			...
			float dr = bottomRadius - topRadius;  // 이 부분 중요함(그람 슈미트)
...
void MeshCylinder::BuildTopCap(vector<VertexMesh>& vertices, vector<UINT>& indices)
{
	...
	for (UINT i = 0; i <= sliceCount; i++)
	{
		...
		vertices.push_back(VertexMesh(x, y, z, u, v, 0, 1, 0, 1, 0, 0));
	}
	vertices.push_back(VertexMesh(0, y, 0, 0.5f, 0.5f, 0, 1, 0, 1, 0, 0));
