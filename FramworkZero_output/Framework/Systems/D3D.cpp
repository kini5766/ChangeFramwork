...
void D3D::Create()
{
	...
	d3dDesc.bVsync = settings.bVsync;
	d3dDesc.ClearFlag = settings.ClearFlag;
}
...
{
	Clear(d3dDesc.ClearFlag, rtv, dsv);
}
	...
	: numerator(0), denominator(1)
{
	...
	CreateBackBuffer(Screen::Width(), Screen::Height());
}
...
bool D3D::EnumerateAdapterOutput(D3DEnumAdapterInfo* adapter_info)
{
	...
	{
		bool isCheck = true;
		isCheck &= display_modes[i].Width == Screen::Width();
		isCheck &= display_modes[i].Height == Screen::Height();
...
void D3D::CreateSwapChainAndDevice()
{
	...
	swapChainDesc.OutputWindow = WinDesc::GetHandle();
	swapChainDesc.SampleDesc.Count = 1;
	swapChainDesc.SampleDesc.Quality = 0;
	swapChainDesc.Windowed = !Screen::IsFullScreen();
