...
map<wstring, Shader::ShaderRef> Shader::shaderMap;

Shader * Shader::Load(wstring file)
{
	return LoadFullName(URI::Shaders + file);
}

Shader * Shader::LoadFullName(wstring fileFullname)
{
	if (shaderMap.count(fileFullname) > 0)
	{
		shaderMap[fileFullname].RefCount++;
		return shaderMap[fileFullname].SRV;
	}

	Shader* result = new Shader(fileFullname);
	ShaderRef desc;
	{
		desc.RefCount++;
		desc.SRV = result;
	...
	shaderMap[fileFullname] = desc;
	return result;
}

void Shader::UnLoad(wstring file)
{
...
{
	for (auto& var : shaderMap)
	{
		SafeDelete(var.second.SRV);
	}
	shaderMap.clear();
}
