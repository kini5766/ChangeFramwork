...
void ShaderSetter::SetSampler(string name, UINT index, ID3D11SamplerState * value)
{
	if (effectValues.count(name) == 0)
	{
		IEffectVariable* effect = new EffectSampler(name, index, value);
	...
	dynamic_cast<EffectSampler*>(effectValues[name])->SetValue(index, value);
}
