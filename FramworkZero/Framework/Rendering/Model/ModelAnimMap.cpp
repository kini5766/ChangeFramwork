...
ModelAnimMap::ModelAnimMap(const ModelAnimDesc& anim)
{
	UINT clipCount = anim.ClipCount;
	UINT boneCount = anim.BoneCount;

	clipBoneMap = new ModelClipTexture(anim);

	KeyframeCount* keyframeCounts = new KeyframeCount[clipCount * boneCount];
	for (UINT c = 0; c < clipCount; c++)
	{
		UINT boneSize = anim.Clips[c]->Bones.size();
		for (UINT b = 0; b < boneSize; b++)
		{
			KeyframeCount& k = keyframeCounts[c * boneCount + b];
			ClipBoneData* bone = anim.Clips[c]->Bones[b];
...
ModelClipTexture::ModelClipTexture(const ModelAnimDesc& anim)
{
	UINT clipCount = anim.ClipCount;
	UINT boneCount = anim.BoneCount;
	...
	for (UINT c = 0; c < clipCount; c++)
	{
		UINT bonesSize = anim.Clips[c]->Bones.size();
		for (UINT b = 0; b < bonesSize; b++)
			maxFrame = max(maxFrame, anim.Clips[c]->Bones[b]->MaxFrame);
	}
	...
	for (UINT c = 0; c < clipCount; c++)
	{
		...
		UINT bonesSize = anim.Clips[c]->Bones.size();
		clipTramsforms[c].SetData(anim.Clips[c]->Bones.data(), bonesSize);
	}
...
void ClipTransform::SetData(ClipBoneData *const* data, UINT BonesSize)
{
	for (UINT b = 0; b < BonesSize; b++)
	{
		UINT size;
